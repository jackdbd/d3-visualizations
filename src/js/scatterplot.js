...

{
  const draw = (data) => {
    ...
    const htmlStr = (d) => {
      const firstStr = `<span>${d.Place}: ${d.Name} (${d.Nationality})</span><br>`;
      ...
      let str = thirdStr;
      if (d.URL) {
        str = `${thirdStr}<br><a href="${d.URL}" target="_blank">Doping Allegations</a>`;
      }
      return str;
    };
    ...
    const bikers = svg.append('g')
      .attr('class', 'bikers');

    bikers.selectAll('.circle').data(data)
      .enter()
  ...
  const cyclingUrl = 'https://raw.githubusercontent.com/FreeCodeCamp/ProjectReferenceData/master/cyclist-data.json';
  d3.json(cyclingUrl, (error, data) => {
    if (error) throw error;
    draw(data);
  });
}
