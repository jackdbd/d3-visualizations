...
{
  const draw = (error, topologyWorld, meteorites) => {
    ...
    const countryScale = d3.scaleOrdinal(d3.schemeCategory20);

    const featureCollection = topojson.feature(topologyWorld, topologyWorld.objects.countries);
    const featuresWorld = featureCollection.features;

    countries.selectAll('path').data(featuresWorld)
      .enter()
      .append('path')
      .attr('class', 'country')
      .attr('d', geoPath)
    ...
    */
    ...
    const massSizeScale = d3.scaleSqrt()
      .domain(d3.extent(masses))
      .range([1, 50]);
    ...
    const massColorScale = d3.scaleThreshold()
      .domain(domain)
      .range(range);
    ...
    const mouseover = (d) => {
      ...
      tooltip.transition().duration(200).style('opacity', 0.9);
      const lat = d3.format('.2f')(d.coords[1]);
      const latitude = lat > 0 ? `${lat}°N` : `${-lat}°S`;
      const long = d3.format('.2f')(d.coords[0]);
    ...
    impacts.selectAll('circle').data(data)
      .enter()
      .append('circle')
  ...
  d3.queue()
    .defer(d3.json, worldMapUrl)
    .defer(d3.json, meteoritesUrl)
    .await(draw);
}
